...
class LinearGradient extends Component<LinearGradientProps> {
  ...
  render() {
    ...
    const nativeLinearProps = omit(props, Object.keys(colorsArray));
...
type AnimatedLinearGradientProps = LinearGradientProps & {
  animation: {
    toValue: number;
    duration: number;
...
class AnimatedLinearGradient extends Component<AnimatedLinearGradientProps, IState> {
  static defaultProps = {
    animation: {
      toValue: 1,
      duration: 500,
  ...
  constructor(props: AnimatedLinearGradientProps) {
    super(props);
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const keys = ['colors'];
    const mutableProps = pick(nextProps, keys);
    const stateToCompare = pick(prevState, keys);
    let animatedColors = prevState.animatedColors;

    animatedColors = AnimatedLinearGradient.animateGradientTransition(
      animatedColors,
      mutableProps.colors,
      prevState.colors,
      nextProps.animation
    ...
    if (!isEqual(mutableProps, stateToCompare)) {
      return {
        ...mutableProps,
        animatedColors,
  ...
  static animateGradientTransition(animatedColors, curColors, prevColors, animation) {
    ...
    if (!isEqual(prevColors, curColors)) {
  ...
  render() {
    ...
    return (
      <AnimLinearGradient {...props} {...animatedColorsProps}>
        {children}
      </AnimLinearGradient>
    );
  }
